import {XmppRegions} from './endpoints';
...
export const parseJid = (jid: string): Jid => {
    ...
    let region = null;
    for(const xmppregion in XmppRegions) {
        if(XmppRegions[xmppregion].domain === domain.split(".")[0]) {
            region = XmppRegions[xmppregion].region;
            break;
        };
    }
    return {local: rest.length === 0 ? null : local, domain, resource, region};
}
